...
function findNetLibsAndroidStatics {
    message "${GREEN}[Net Libs]${NORMAL}"
    missingNetLib=no
    for i in ${@} ; do
    ...
    if [ "$missingNetLib" == "yes" ] ; then
        echo " ${YELLOW}consider:${NORMAL} \
./utils/scripts/prepare_netlibs.sh"
    fi
}
...
function prepareNetlibsScript {
    message "${GREEN}[NetLibs Setup]${NORMAL}"
    if  [ ! -d external/netlibs/czmq-3.0.0 ]  || \
        [ ! -d external/netlibs/zeromq-4.1.0 ] ; then
        writeStatus "  - NetLibs not configured. Running script now." 1
        ./utils/scripts/prepare_netlibs.sh
    else
        writeStatus "  - NetLibs already configured " 0
    fi
}
...
findNetLibsAndroidStatics libzmq.a libczmq.a
...
if [[ $hasFailed ]] ; then
    ...
    echo "Necessary Android symlink paths are not configured until script is successful."
else
    ...
    ANGELSCRIPT_DIR="../../external/angelscript"
    NETLIBS_DIR="../../external/netlibs"
    ...
    [[ ! -e $PATH_JNI/angelscript ]]&& ln -s $ANGELSCRIPT_DIR    $PATH_JNI/angelscript
    [[ ! -e $PATH_JNI/netlibs ]]    && ln -s $NETLIBS_DIR       $PATH_JNI/netlibs
